...
public class LoGEdgeSharpening {
    ...
    public static int[][] ProcessImage( int [][] inputPixels){
        ...
        double [][]G=new double[(int)(2*r)][(int)(2*r)];
        int[][] sharpen={
            {0, 1,  0},
            {1, -4, 1},
            {0, 1,  0}
        ...
        for(int i=-rs;i<=rs;i++)
            for(int j=-rs;j<=rs;j++){
                G[i+rs][j+rs]=Math.pow(Math.E,-(i *i +j *j )/(2*r*r));
                gsum=gsum+G[i+rs][j+rs];
            }
        for(int i=-rs;i<=rs;i++)
            for(int j=-rs;j<=rs;j++){
                G[i+rs][j+rs]=G[i+rs][j +rs]/gsum;
            }
	...
	public static void main(String[] args){
		if (args.length < 1){
			System.out.printf("Usage: ImageProcessor205BW <input image> <output name> <w> <radius>\n");
			return;
		}
        if (args.length==3){
