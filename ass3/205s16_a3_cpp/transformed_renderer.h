...
public:
	...
	void draw_image(SDL_Texture *image,SDL_Rect m_pSrcRect,SDL_Rect m_pTargetRect){
		Sint16 new_vx;
		Sint16 new_vy;
		
		TransformVector(m_pTargetRect.x,m_pTargetRect.y,new_vx,new_vy);
		m_pTargetRect.x=new_vx;
		m_pTargetRect.y=new_vy;
		
		SDL_RenderCopy(renderer,image,&m_pSrcRect,&m_pTargetRect);
	}
