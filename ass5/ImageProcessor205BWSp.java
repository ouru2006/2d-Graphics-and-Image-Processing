...
public class ImageProcessor205BWSp {
    ...
    public static int[][] ProcessImage( int [][] inputPixels){
        ...
        for (int x = 0; x < width; x++)
            for (int y = 0; y < height; y++)
                newI[x][y]=Twirl(inputPixels,x,y,xc,yc,rad,ref);
    ...
    private static int Twirl(int[][] I, int x, int y, double xc,double yc,double rad, double ref){

        double dx=x-xc;
        double dy=y-yc;
        double r=Math.sqrt(dx*dx+dy*dy);
        double z=Math.sqrt(rad*rad-r*r);
        if(r<=rad){
            double bx=(1-1/ref)*Math.asin(dx/Math.sqrt(dx*dx+z*z));
            double by=(1-1/ref)*Math.asin(dy/Math.sqrt(dy*dy+z*z));
            
            x=(int)(x-z*Math.tan(bx));
            y=(int)(y-z*Math.tan(by));
        }
